...
/// (<see cref="SourceCommentScanner"/>) joined by identity — comments cannot be read from the binary.
/// </summary>
internal static class TemplateBuilder
{
    ...
    /// comment is looked up by its key and default. <paramref name="includeAnnotations"/> folds in strings carried
    ...
    /// sites take precedence over an annotation on the same (category, key).</summary>
    public static Catalog? Build(AssemblyStringExtractor extractor, string assemblyPath, string sourceLanguage, CommentIndex? comments = null, bool includeAnnotations = true)
    {
