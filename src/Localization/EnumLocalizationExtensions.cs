...
/// of the build-time annotation extraction. It reads, by reflection, a <see cref="LocalizedAttribute"/> (key and
/// default in one) or, failing that, a <c>[Display(Name = …)]</c> literal with a
/// <see cref="LocalizedDisplayNameAttribute"/> twin supplying its default, then resolves it through the localizer
/// under the enum type's category (the <c>(category, key, default)</c> the extractor wrote). The precedence is
/// <see cref="MemberLocalizationExtensions"/>'s, so an enum member reads exactly as any other member does. A
/// member with no display annotation, or a composite/undefined value, renders as its name — the same as
/// <see cref="object.ToString"/>. Reflection over the member's attributes is inherent to reading attributes at
/// runtime; this and <see cref="MemberLocalizationExtensions"/> are the only places the library does it.
/// </summary>
public static class EnumLocalizationExtensions
{
    ...
    private static string Resolve(Enum value, LocalizationContext context)
    {
        ...
        if (field is null || MemberLocalizationExtensions.DisplayNameAnnotation(field) is not { } annotation)
        {
        ...
        // An enum member is a field, so it reads its annotation through the same precedence every other member
        // uses — [Localized] first, then the system attribute with its twin. Extraction lifts both off a field
        // alike, so anything but one shared rule would ship a translation the lookup never asks for.
