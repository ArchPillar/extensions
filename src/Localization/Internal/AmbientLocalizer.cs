...
/// <summary>
/// The global-namespace ambient localizer behind <see cref="Localization.Default"/>: every call resolves
...
internal sealed class AmbientLocalizer : ILocalizer
{
    ...
    public string Translate(string key, string defaultMessage, params (string Name, object? Value)[] arguments)
    {
        ...
        Localization.EnsureCulture(culture);
        return Localization.Current.Translate(culture, key, defaultMessage, context: null, arguments);
    }
    ...
    public string Translate(string key, string defaultMessage, string context, params (string Name, object? Value)[] arguments)
    {
        CultureInfo culture = CultureInfo.CurrentUICulture;
        Localization.EnsureCulture(culture);
        return Localization.Current.Translate(culture, key, defaultMessage, context, arguments);
    }
}

/// <summary>
/// The category-scoped ambient localizer behind <see cref="Localization.For{T}"/>: its category is the full
...
internal sealed class AmbientCategoryLocalizer<T> : ILocalizer<T>
{
    ...
    public string Translate(string key, string defaultMessage, params (string Name, object? Value)[] arguments)
    {
        Localization.EnsureCulture(CultureInfo.CurrentUICulture);
        return Localization.Current.TranslateInCategory(_category, key, defaultMessage, context: null, arguments);
    }
    ...
    {
        Localization.EnsureCulture(CultureInfo.CurrentUICulture);
        return Localization.Current.TranslateInCategory(_category, key, defaultMessage, context, arguments);
    }
}
