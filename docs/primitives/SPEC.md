...
`Primitives` is a collection of small, dependency-free .NET utilities. This document is the specification for the first primitive in the library: **`Pipeline<T>`**.

Every primitive that joins this package is specified in its own section of this file (or its own spec document linked from here). The library as a whole has one unifying rule: **each primitive is self-contained** — no shared machinery, no cross-primitive coupling, no runtime reflection. You depend on a primitive; you don't depend on a framework.
...
Lives in the companion package `ArchPillar.Extensions.Primitives.DependencyInjection`.
