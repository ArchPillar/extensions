...
- **No batch handler.** `SendBatchAsync` does not construct a batch context. It iterates the input list and calls `SendAsync(command)` per item. Each iteration runs the item's full pipeline pass — **validation then handler** — to completion before the next item is considered. The loop bails on the first failure (validation *or* handler) and returns that failure verbatim; items already processed are not rolled back. On full success it returns success (or, in the result-bearing form, the aggregate list of payloads in input order). There is no batch-level atomicity in this mode — items are independent dispatches invoked in a loop.
