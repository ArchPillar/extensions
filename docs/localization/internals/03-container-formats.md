...
- **Encoding:** UTF-8 without byte-order-mark, `\n` line endings, on every format. On write the tooling re-encodes to the line ending the path should use, so a repository that keeps CRLF on disk (Git `autocrlf`, a `text` attribute, or a declared `end_of_line`) sees only the lines that actually changed rather than a whole-file diff. An existing file's own convention wins — it is fact rather than declaration, and matching it is what keeps the diff minimal; for a catalog that does not exist yet, `.editorconfig`'s `end_of_line` seeds it, falling back to the canonical `\n`.
