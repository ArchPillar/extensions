# 03 — Container Formats (Provider Abstraction + Portable Object + XLIFF 2.1 + Application Resource Bundle + APLOC deploy bundle)
...
One interface that the reconciler (spec 02) and the runtime (spec 05) use to read and write translation files, so neither knows or cares which on-disk format is in play. Four implementations: three **authoring/interchange** formats — Portable Object for simple/community handoff, the Extensible Markup Language Localization Interchange File Format (XLIFF) version 2.1 for professional handoff, and the Application Resource Bundle (ARB) as the JavaScript Object Notation answer (spec-backed and ICU-native) — plus one **deploy-only** format, APLOC, the compact runtime bundle the publish-time merge emits. Generic untyped JavaScript Object Notation as an *authoring* format is deliberately not implemented — it has no standard schema and therefore no interoperable translator tooling; APLOC is not that (it is not a handoff format at all, it is the compiled runtime artifact, the equivalent of a `.resources` blob).
...
public interface ITranslationFormat
{
    ...
    Catalog Read(Stream input);
    Task WriteAsync(Stream output, Catalog catalog, CatalogWriteOptions? options = null, CancellationToken cancellationToken = default);
}
...
- **Source text:** ARB has a *predefined* `source_text` attribute — "the source of the text from where this message is translated from" — so use it (not a homegrown field) to preserve the original. ARB holds one value per key, so once an entry is translated its value is the translation and the source would otherwise be lost (from the file *and* the in-memory model, which collapses source and translation to the one value on read). Write `source_text` whenever the value is a *distinct translation* (the value is not the source itself); an untranslated entry's value already is the source, so it is omitted. On read, the source is `source_text` when present, else the value. Being a standard attribute, tools that understand it recognise it rather than ignoring it.
...
## APLOC provider (`Formats` — the deploy bundle)

APLOC (`.aploc`, format id `aploc`) is the **default published-bundle** format (spec 06), not an authoring format. It exists to be the smallest, cheapest thing a Blazor WebAssembly client parses, so it carries only what the runtime resolves — the translated value per entry — and nothing of the authoring workflow.

- **Container:** minified JSON (pretty when written by `convert` for inspection). It stays JSON deliberately: the runtime reads it with the same `System.Text.Json` reader the ARB provider uses (no new parser), and JSON string escaping makes a value with a newline, quote or `=` safe with no format-specific escaping scheme to get wrong.
- **Key model — the reason it exists:** the category is folded into a **nested object tree** rather than repeated in a flat `Category::Key` member. Each dot-segment of the category is one nested object; a node's **own keys are plain string members of that object** (member name is the key, value the ICU message), sitting side by side with the child-namespace objects. So `Acme.Widgets.ButtonLabels::{Back,Cancel,…}` writes `Acme` once, `Widgets` once, `ButtonLabels` once, then `"Back": …, "Cancel": …` directly — where ARB repeats the 44-character category on every member *and* carries a redundant `@…` metadata twin. **Uncategorized (global) entries are the root object's own string members** (there is no separate global bucket). `@@locale` at the root carries the culture (as in ARB). **Only the category nests — a key is a member taken verbatim**, so a dotted key (`home.title`) is one key, never split.
- **The `"@"` apex — used only on collision, DNS-style.** Keys and child namespaces share one member namespace, so the rare key whose name equals a child namespace cannot also be a plain member. Following the DNS-zone convention where `@` names the record *at the node itself*, that key is written as the child object's `"@"` member: `"Models": { "Size": { "@": "Größe", "Large": "Groß" } }` — the key `Size` in category `Acme.Models` and the category `Acme.Models.Size` share one `Size` object, so the segment name is written **once**. On read, a node's `"@"` string is attributed to the *parent* category under the node's own segment name. A real bundle carries **no `"@"` at all** — categories are type full-names, and a name cannot be both a type and a namespace, so no category is a dotted prefix of another; the apex exists purely so a hand-built pathological catalog still round-trips.
- **Reserved names.** `@` is the format's sigil, so it is not a usable key: a key named exactly `@` (the apex) or starting with `@@` (the header prefix) is rejected on write with a clear error, rather than emitting a bundle that would read back as something else. A key merely *starting* with a single `@` (`@id`) is unambiguous and stays an ordinary plain member.
- **Lossy by design:** source text, state, comments, references, placeholders and fingerprints are all dropped on write; an entry reads back as a complete `Translated` translation. It round-trips a *translation*, not an authoring workflow — use `convert` to move a catalog back to XLIFF/ARB/PO for editing. `Read` still works (a hand-inspected or hot-patched bundle loads), which is why it implements the full provider interface rather than a write-only path.
...
- **Minified publish bundle (`Minify`):** the publish-time `merge` write drops insignificant whitespace and every annotation the runtime does not read (state, fingerprint, `source_text`, comments, references, placeholders). APLOC (the default bundle format) drops all of it structurally — it stores only the value, in a nested tree — so `Minify` there is just whitespace. For ARB the only metadata kept is `x-category`, because the reader needs it to recover the bare key from the category-qualified member name (`Unqualify` strips a *known* prefix). An entry with no category needs no `@key` object at all. An absent `x-state` therefore reads back as a usable translation (`Translated`), not as untranslated; only an explicit `NeedsTranslation` marks a placeholder. XLIFF drops its indentation (its structure is all meaningful); PO is line-oriented and already compact.
...
- **Encoding:** UTF-8 without byte-order-mark, `\n` line endings, on every format. A repository that wants its catalogs in CRLF declares it the way it does for any other file type — an `end_of_line` in `.editorconfig`, which may target the catalog extensions specifically — and the tooling writes what is declared, falling back to the canonical `\n`. The declaration is the only authority: the convention a catalog happens to be checked out with is a repository configuration matter, and inferring it from the file would silently work around that forever. Nothing is normalized on its own schedule, because nothing is written unless the content changed.
- **No no-op writes:** an unchanged catalog is not written at all — an identical rewrite still moves the modification time that incremental builds, file watchers and caches key off. `sync` and `extract` decide from the reconcile that produced the catalog: both the reconciled and the previously parsed catalog are serialized with identical options and compared in memory, so the decision costs no read of the file beyond the parse it already did. The comparison is of *serialized* forms, not the models — PO and XLIFF do not persist placeholders, so comparing models would report a difference the file could never hold and rewrite every catalog on every run. Generated output built from elsewhere (the publish bundles, the manifest) has no parsed prior version, so it compares against the destination's bytes instead: one read, no parse.
- **Formatting is never "fixed" on its own:** because the decision is content-level, a catalog whose formatting the tooling did not produce (odd line endings, a byte-order-mark, hand reflowing) is left alone while its content is up to date. `sync --check` therefore gates on content drift, not byte-identity with a fresh serialize.
