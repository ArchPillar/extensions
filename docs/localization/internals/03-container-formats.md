...
- **State, references, source, previous-source:** ARB defines no native fields for these. Persist them under custom `x-`-prefixed metadata inside `"@key"` (e.g., `"x-state"`, `"x-references"`, `"x-source"`, `"x-previous-source"`, `"x-source-fingerprint"`), which the spec permits and tools ignore. The fingerprint (`x-source-fingerprint`) must be persisted because the reconciler needs it and ARB has nowhere else to put it. **`x-source`** is written whenever the entry's value is a *distinct translation* (i.e., the value is not the source itself): ARB holds one value per key, so without it a translated entry would lose the source text the translator is working from — `x-source` keeps the original visible beside the translation and round-trips it. An untranslated entry's value already is the source, so `x-source` is omitted; on read, the source is `x-source` when present, else the value.
