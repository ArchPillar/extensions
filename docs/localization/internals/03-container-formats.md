...
- **No no-op writes:** a catalog whose bytes would be unchanged is not written at all. An identical rewrite still moves the file's modification time, and incremental builds, file watchers and caches key off it — so the per-build `extract`/`sync` must leave an up-to-date catalog completely untouched, not merely byte-identical.
