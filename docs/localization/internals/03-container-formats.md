...
- **Key model:** our symbolic `Key` is the JavaScript Object Notation key directly — ARB is key-based, so this is the natural mapping (like XLIFF, and unlike Portable Object). Source text is the value in the source-locale `.arb`; translations are the values in per-locale `.arb` files.
