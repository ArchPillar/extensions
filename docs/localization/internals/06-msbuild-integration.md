...
- [ ] Publishing a **server host that references a WebAssembly app** (which references a localized library) ships the merged per-culture bundle(s) and `apl-catalogs.json` under `wwwroot/Translations`, and does **not** leave the library's raw per-culture catalogs under `wwwroot/_content/<library>/Translations/` nor any beside-the-binary catalog copy at the host output root.
