...
For a host with no readable file system — a Blazor WebAssembly client, which fetches its catalogs over HTTP rather than reading them from disk (spec 05) — the build additionally produces a **catalog manifest** (`apl-catalogs.json`) listing every non-source catalog by culture and file name; over HTTP there is no directory to enumerate, so the runtime reads this index to discover what to fetch. Rather than a post-build write (which lands after the static-web-asset pipeline has already resolved and fingerprinted the served assets), the manifest is wired **into** that pipeline: a target appended to `ResolveStaticWebAssetsInputsDependsOn` (reached via `PrepareForRun`, late enough that the tool has been built and the `wwwroot` catalogs are discovered) generates the manifest into the intermediate output and registers it through the SDK's `DefineStaticWebAssets`/`DefineStaticWebAssetEndpoints` tasks as a `Computed` asset at `Translations/apl-catalogs.json` — the same mechanism the SDK uses for its own JS-initializer-module manifest. It is therefore fingerprinted, compressed, and served like any `wwwroot` asset, and (registered as `AssetKind="All"`) flows into the publish manifest as well, so `dotnet build` and `dotnet publish` both serve it without anything committed to the source tree. Emission is on by default (`ArchPillarLocalizationEmitManifest`); `ResolveStaticWebAssetsInputsDependsOn` exists only in Razor/Blazor projects, so this is inert elsewhere (a console or server app loads catalogs from the file system and needs no manifest).
