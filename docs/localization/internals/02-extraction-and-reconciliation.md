...
The source-language template is produced by the **tool**, not the generator — a generator cannot do file I/O, and reading the built assembly's **IL** (Decision D-K) lets extraction also cover strings the syntax-level detector never sees (Razor/Blazor/MVC generated code). `TemplateBuilder` runs the IL extractor (`AssemblyStringExtractor`, Mono.Cecil) over each in-scope assembly and recognises a translatable call by the **same `[Translatable]`/`[TranslationDefault]` parameter attributes** the source-level detector reads — so the native `Translate`, the `L(...)` marker, and user wrappers (methods or indexers) are all found from metadata alone, without loading code. The category is taken from a `[TranslationScope]`-marked argument found first on the called method's own type parameters (so a static/extension method can define its scope, e.g. `Label<T>(...)`) and otherwise by walking the receiver's own type and its base types/interfaces (so `ILocalizer<T>` and `Localized<TSelf>` resolve identically to the runtime). The receiver is argument 0 whether the call is an instance call or a reduced extension method, and a value-type receiver held in an `in`/`ref` parameter is unwrapped from its managed reference, so neither shape silently loses its scope. The resulting name is normalized to the form the runtime asks for (`CategoryName.Of`) and the Roslyn detector writes — the open-generic name for a generic type (the arity backtick without the type arguments) and `+` for a nested type, where Cecil writes the arguments and `/` — because a category the runtime never produces silently falls back to the in-code default. An assembly with no translatable call sites yields **no template** — empty files are never written.
...
- **`status [--detail overall|language|project|matrix]`** — report translation coverage. Every project/language pair is measured once (`TranslationProgress.Measure`: a catalog against its project's template, `Translated` + `Final` as done, `NeedsReview` its own bucket, and any template key the catalog lacks as missing); `--detail` only chooses how far those pairs are summed before rendering — the counts are additive, which is what lets one measurement answer all four levels. Default `overall` — the headline number is the question `status` is usually asked; the narrower levels say where the remainder is. With no target catalogs yet there is nothing to measure and the report degrades to the strings-per-assembly listing, which is also how `status` answers "which assemblies have strings?" before any language exists.
