...
- **No file system: load over HTTP (Blazor WebAssembly).** A browser has no readable file system, so the directory source finds nothing; `HttpCatalogLoaderExtensions` is its client-side counterpart. `AddCatalogsFromManifestAsync(httpClient)` fetches the build-emitted manifest (`apl-catalogs.json`, spec 06) and then fetches and layers in every catalog it lists; the lower-level `AddCatalogsFromHttpAsync(httpClient, uris)` takes an explicit URI list. Each fetched file's format is detected by extension and parsed by the same providers (spec 03), and a missing, unrecognised, or malformed asset is skipped so the app degrades to its in-code defaults. The manifest is what directory enumeration is on disk — over HTTP there is no directory to list — and the build registers it (and the catalogs) as static web assets through the Razor pipeline (spec 06), so it is served, fingerprinted, and compressed in both the development build and the published output. These extensions live in the core runtime (`HttpClient` is a BCL type, so it stays dependency-free); serving the catalogs from an ASP.NET Core host is the `ArchPillar.Extensions.Localization.AspNetCore` package.
