...
The test suite includes PostgreSQL integration tests (via Testcontainers) that verify SQL translation against a real database. **Docker and PostgreSQL are available in cloud environments** — always run the full test suite (`dotnet test tests/Mapper.Tests`) including PostgreSQL tests. Do not skip or filter out Postgres tests.
