...
- ArchPillar's `Translations/de.arb` entry winning where it has one (`Welcome`), falling through to the `.resx` where it doesn't (`Goodbye`), then to the name (`Help`).
...
```
Prints `en` and `de` blocks: `Welcome` comes from the new `de.arb`, `Goodbye` from the legacy `.resx`, and `Help` falls back to the key; ends with the `L(...)` marker line.
